...
#
# Read once at import time: MLX_GDN_PACKED must be set before mlx_lm is
# imported, it cannot be flipped from inside a running process.
...
# Lifted verbatim from _make_gated_delta_kernel(vectorized=False)'s g_compute,
# so the fused gate arithmetic below is the same expression the generic kernel
# evaluates. Produces g_val and beta_val.
_FUSED_G = """
          float a_val = static_cast<float>(a_[hv_idx]);
          float dt_val = static_cast<float>(dt_bias[hv_idx]);
          float x_g = a_val + dt_val;
...
def _gated_delta_kernel_impl(
    ...
    *,
    ...
    # (There is no g.dtype condition here as in the unfused upstream variant:
    # g is not an array in this API, it is computed in-kernel in float32.)
    packed_eligible = (
        mask is None
        and a.ndim == 3
        and Dk == 128
...
def gated_delta_kernel_packed(
    ...
    """Force the packed kernel regardless of the env switch (test use)."""
    assert mask is None, "the packed kernel has no masked variant"
    B, T, Hk, Dk = k.shape
    Hv, Dv = v.shape[2:]
    return _gated_delta_kernel_packed(
        inputs=[q, k, v, a, b, A_log, dt_bias, state, T],
        template=[
            ("InT", q.dtype),
            ("StT", state.dtype),
...
def gated_delta_kernel_unpacked(
    ...
    return _gated_delta_kernel_impl(
        q, k, v, a, b, A_log, dt_bias, state, mask, allow_packed=False
    )
...
def gated_delta_kernel(
    ...
    return _gated_delta_kernel_impl(
        q, k, v, a, b, A_log, dt_bias, state, mask, allow_packed=True
    )
